...
export function useCollabEditor({ name, active, textareaRef }) {
  ...
  // Holds the cursor position we want to restore after a remote update
  ...
  const transformPair = useCallback((remoteOp, localOp) => {
    const newRemoteArr = transform(localOp, remoteOp);
    const newLocalArr = transform(remoteOp, localOp);
  ...
  const applyRemoteOp = useCallback((remoteOp, serverRev) => {
    ...
    // ---------- CURSOR FIX: capture local cursor before applying ops ----------
    const textarea = textareaRef.current;
    let savedCursor = null;
    if (textarea && document.activeElement === textarea) {
      savedCursor = { start: textarea.selectionStart, end: textarea.selectionEnd };
    }
    ...
    // ---------- Transform the saved cursor through the applied ops ----------
    if (savedCursor) {
      let newStart = savedCursor.start;
      let newEnd = savedCursor.end;
      for (const op of ops) {
        if (op) {
          newStart = transformCursor(newStart, op);
          newEnd = transformCursor(newEnd, op);
        }
      }
      pendingCursorRef.current = { start: newStart, end: newEnd };
    }
  ...
  // ---------- Restore local cursor after a remote text update ----------
  useLayoutEffect(() => {
    if (pendingCursorRef.current) {
      const textarea = textareaRef.current;
      if (textarea && document.activeElement === textarea) {
        textarea.selectionStart = pendingCursorRef.current.start;
        textarea.selectionEnd = pendingCursorRef.current.end;
      }
